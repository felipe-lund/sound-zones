...
# def plot_filter_impulse_responses(g_full, fs, nfft):
#     # Convert filters from Frequency Domain to Time Domain
#     # g_time shape will be (num_speakers, nfft)
#     g_time = np.fft.irfft(g_full, n=nfft, axis=1)
    
#     t_filter = np.arange(nfft) / fs
    
#     plt.figure(figsize=(10, 5))
#     # Plotting the first 5 speakers to see the behavior
#     for i in range(min(5, g_time.shape[0])):
#         plt.plot(t_filter * 1000, g_time[i, :], label=f'Speaker {i}')
    
#     plt.title("Time Domain Impulse Response of Filters (g)")
#     plt.xlabel("Time [ms]")
...
# plot_filter_impulse_responses(g_full, fs, nfft)
